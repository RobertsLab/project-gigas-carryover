...
create_boxplot <- function(data, comparison, t_test_results) {
  ...
  boxplot <- ggplot(filtered_data, aes(x = Target, y = delta_Cq, fill = life.stage)) +
    ...
    ylim(y_limits) +
    labs(x = "Target", y = "Delta Cq", title = comparison) +
...
for (comparison in unique_comparisons) {
  create_boxplot(delta_Cq_df, comparison, life_stage_t_test_results_df)
}
```
