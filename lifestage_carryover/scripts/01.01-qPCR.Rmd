...
NOTE: Below is the full set of groups for the _entire experiment_. For the current qPCR analysis, `seed` and `spat` do _not_ have acute treatments; just conditioning treatments.
...
### Life stage
```{r delta-delta-Cq-life-stage, eval=TRUE}
...
```{r fold-change-life-stage, eval=TRUE}
...
```{r bar-plots-fold-change-lifestages, eval=TRUE}
...
### HSP70: Significant effect of conditioning x acute treatment. 
```{r, eval=TRUE}
plot3<-delta_Cq_df%>%
  filter(Target=="HSP70")%>%
...
### HSP90: Significant effect of lifestage x acute treatment, lifestage x conditioning treatment, acute treatment, and lifestage. 
```{r, eval=TRUE}
plot4<-delta_Cq_df%>%
  filter(Target=="HSP90")%>%
...
### cGAS: Significant effect of lifestage x acute treatment and lifestage.  
```{r, eval=TRUE}
plot5<-delta_Cq_df%>%
  filter(Target=="cGAS")%>%
...
### Citrate synthase: Significant effect of acute treatment and lifestage x acute treatment.  
...
### Viperin: Significant effect of lifestage and lifestage x acute treatment.  
...
ggsave(plots, filename="lifestage_carryover/output/qPCR/spat-heat-shock.png", width=8, height=4)
```

## Conclusions 

### Effects of conditioning on acute stress response   
...
### Effects of acute stress and lifestage 
